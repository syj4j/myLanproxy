...
public class ClientChannelMannager {
    ...
    public static void borrowProxyChanel(ServerBootstrap bootstrap, final ProxyChannelBorrowListener borrowListener) {
    	Channel channel = proxyChannelPool.poll();
    	if (channel != null) {
    		borrowListener.success(channel);
    		return;
    	}else{
    		borrowListener.error(new RuntimeException("proxyChannelPool is null!"));
    	}
    }

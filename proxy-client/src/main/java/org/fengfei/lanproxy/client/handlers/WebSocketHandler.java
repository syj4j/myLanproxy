...
public class WebSocketHandler extends ChannelInboundHandlerAdapter {
	...
	private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) throws Exception {
		...
		// logger.warn("exist channel for key {}, {}", clientKey,
		// ctx.channel());
		// ctx.channel().close();
		// }else{
		// map.put("clientKey", ctx.channel());
		// }
		//
		// // 连接成功，向服务器发送客户端认证信息（clientKey）
		// ClientChannelMannager.setCmdChannel(ctx.channel());
		// logger.info("connect proxy server success, {}", ctx.channel());
	...
	private void handleWebSocketRequest(ChannelHandlerContext ctx, WebSocketFrame req) {
		...
		if (req instanceof TextWebSocketFrame) {
			if("auth".equals(((TextWebSocketFrame) req).text())){
				// 连接成功，向服务器发送客户端认证信息（clientKey）
				//cmdchannel是发送验证消息的channel,传输消息的channel有多条,验证的只有一条
				ClientChannelMannager.setCmdChannel(ctx.channel());
				...
				logger.info("proxy channel connect success, {}", ctx.channel());

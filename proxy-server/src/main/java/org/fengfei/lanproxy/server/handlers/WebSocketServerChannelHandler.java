...
public class WebSocketServerChannelHandler extends SimpleChannelInboundHandler<ProxyMessage> {
    ...
    private void handleConnectMessage(ChannelHandlerContext ctx, ProxyMessage proxyMessage) {
        ...
        if (userChannel != null) {
            ...
            if(tokens.length==3){
            	userChannel.attr(Constants.USE_COMPRESS).set(Boolean.valueOf(tokens[2]));
            }else{
            	userChannel.attr(Constants.USE_COMPRESS).set(false);
            }
    ...
    private void handleAuthMessage(ChannelHandlerContext ctx, ProxyMessage proxyMessage) {
        ...
        if (channel != null) {
            logger.info("exist channel for key {}, {}", clientKey, channel);
            ctx.channel().close();
            return;
        }
